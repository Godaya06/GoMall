      ...
      <div className="container">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {filtered.map((phone, i) => {
            const { original, current, discountPercent } = getPhonePricing(phone);
            return (
              <motion.div
                key={phone.id}
              ...
              >
                ...
                <span className="absolute top-4 left-4 px-2 py-0.5 rounded-full bg-destructive text-destructive-foreground text-[10px] font-bold">
                  -{discountPercent}%
                </span>
                <Link to={`/product/${phone.id}`} className="block">
                  ...
                  <div className="flex items-baseline gap-2 mb-3 flex-wrap">
                    <p className="text-primary font-bold text-xl">KES {current.toLocaleString()}</p>
                    <p className="text-muted-foreground line-through text-sm">KES {original.toLocaleString()}</p>
                  </div>
                </Link>
                ...
                <Button
                  ...
                  onClick={() => addItem({ id: phone.id, name: phone.name, price: current, image: phone.image })}
                >
